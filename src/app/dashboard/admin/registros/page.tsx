...
function AdminRegistrosContent() {
  ...
  useEffect(() => {
    fetch("/api/registros", { cache: "no-store" })
      ...
      .then(data => {
        ...
        else setRegistros(data.registros || [])
      })
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: NAVY }}>Todos los Registros GPS</h1>
          <p className="text-sm text-gray-500 mt-1">
            Vista global del sistema Halconsat — {cargando ? "cargando…" : `${filtrados.length} de ${registros.length}`} registros
          </p>
        </div>
      ...
      {!error && !cargando && filtrados.length === 0 && (
        <div className="bg-white rounded-xl border shadow-sm p-10 text-center space-y-2">
          <span className="text-4xl">📭</span>
          <p className="text-gray-500 font-medium">
            {registros.length === 0 ? "Aún no hay registros en el sistema" : "Sin resultados para este filtro"}
          </p>
        </div>
      )}
